...
公开列表地址为复数形式 `/galleries`，详情地址继续使用单数形式 `/gallery/:slug`；旧列表地址 `/gallery` 会永久跳转到 `/galleries`。每个 Gallery 自动以封面优先、照片排序靠前的规则选择最多三张预览图，并从卡片进入详情页。列表和详情页都与站点 Logo、Post 的 `1200px` 基线对齐；列表采用响应式双列留白布局，不使用渐变或厚重装饰。
