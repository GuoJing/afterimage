...
- 三语言留言板：登录或匿名留言、审核发布、IP 限流与安全纯文本过滤
- 全局导航管理，支持自定义名称、URL、排序及当前/新页面打开
- 后台用户管理：新增、编辑、封禁、0–5 会员等级及邮件密码重置
- 三语会员邮件订阅偏好，以及按文章手动群发、成功去重和单用户强制重发
...
公开地址为 `/guestbook`，支持中文、English、日本語，并按当前站点语言显示。登录会员只需填写留言内容，系统使用账号昵称；未登录访客需要同时填写用户名和留言。所有留言默认保存为 `pending`，审核前不会出现在公开页面。后台侧栏的“留言管理”可以审核通过或永久删除留言，审核通过的留言按时间倒序展示，前台每页 10 条。

留言只允许纯文本，服务端会移除 HTML、`script`、`style` 等标签和危险控制字符，模板输出继续执行 HTML 转义，所有数据库操作均使用参数绑定。同一 IP 每分钟只能提交一次；数据库只保存基于服务端密钥计算的 IP 摘要，不保存明文 IP。应用启动时自动创建 `guestbook_messages` 表和查询索引，无需手动执行 SQLite 迁移。
...
登录会员可以在会员中心保存网站默认语言。这个偏好为未来的全站语言功能预留，目前不会修改页面语言、语言 Cookie 或路由，只用于选择文章推送邮件的语言；新注册用户自动使用 `DEFAULT_LOCALE`。会员也可以从会员中心进入 `/subscribe`，分别管理“新文章推送”“Newsletter”和“活动推送”。三类邮件对正常会员默认开启，系统只持久化会员主动关闭的类别；重新开启时会删除相应退订记录。页面支持中文、English、日本語，接收地址固定使用账号邮箱，不接受表单传入的其他邮箱。目前 Newsletter 与活动推送只保留偏好数据，尚未接入实际群发；新文章推送已经可以使用。
...
邮件根据会员账号保存的默认语言选择文章翻译；不存在时回退 `DEFAULT_LOCALE`，再回退文章的第一个可用翻译。邮件包含文章标题、摘要、正文、阅读全文链接、订阅设置链接和版权信息。只有已发布文章可以发送，草稿会被后台拒绝。应用启动时自动为 `users` 增加 `preferred_locale` 字段，并创建 `user_subscription_opt_outs` 和 `post_email_deliveries` 表，无需手动执行 SQLite 迁移。
