...
登录会员可以从会员中心进入 `/subscribe`，分别保存“新文章推送”“Newsletter”和“活动推送”三个偏好。页面支持中文、English、日本語，接收地址固定使用账号邮箱，不接受表单传入的其他邮箱。目前 Newsletter 与活动推送只保留已生效的偏好数据，尚未接入实际群发；新文章推送已经可以使用。
...
邮件根据会员保存订阅偏好时使用的语言选择文章翻译；不存在时回退默认语言，再回退文章的第一个可用翻译。邮件包含文章标题、摘要、正文、阅读全文链接、订阅设置链接和版权信息。只有已发布文章可以发送，草稿会被后台拒绝。应用启动时自动创建 `user_subscriptions` 和 `post_email_deliveries` 表，无需手动执行 SQLite 迁移。
