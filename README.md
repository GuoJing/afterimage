...
登录会员可以从会员中心进入 `/subscribe`，分别管理“新文章推送”“Newsletter”和“活动推送”三个偏好。三类邮件对正常会员默认开启，系统只持久化会员主动关闭的类别；重新开启时会删除相应退订记录。页面支持中文、English、日本語，接收地址固定使用账号邮箱，不接受表单传入的其他邮箱。目前 Newsletter 与活动推送只保留偏好数据，尚未接入实际群发；新文章推送已经可以使用。
