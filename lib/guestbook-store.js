...
const AUTHOR_DISPLAY_WIDTH_LIMIT = 60;
const AUTHOR_PATTERN = /^(?:[\p{L}\p{N}]\p{M}*)+$/u;
...
export class GuestbookStore {
  ...
  getApproved(limit, offset) {
    return this.db.prepare(`
      SELECT m.id, m.user_id, m.author_name, m.content, m.submitted_at, u.avatar_url
...
export function sanitizeGuestbookAuthor(value) {
  ...
  if (!source
    || Array.from(source).length > AUTHOR_LIMIT
    || !AUTHOR_PATTERN.test(source)
    || guestbookAuthorDisplayWidth(source) > AUTHOR_DISPLAY_WIDTH_LIMIT) {
    throw new GuestbookValidationError('INVALID_AUTHOR');
  }
  return source;
}

export function guestbookAuthorDisplayWidth(value) {
  return Array.from(String(value || '').normalize('NFKC')).reduce((width, character) => {
...
export const guestbookLimits = {
  author: AUTHOR_LIMIT,
  authorDisplayWidth: AUTHOR_DISPLAY_WIDTH_LIMIT,
