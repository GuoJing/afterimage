...
export class SubscriptionStore {
  constructor(db, { defaultLocale = 'zh' } = {}) {
    ...
    this.getSettingStatement = db.prepare('SELECT locale, updated_at FROM user_subscription_settings WHERE user_id = ?');
    this.getOptOutsStatement = db.prepare('SELECT subscription_type FROM user_subscription_opt_outs WHERE user_id = ?');
    this.saveSettingStatement = db.prepare(`
      INSERT INTO user_subscription_settings (user_id, locale) VALUES (?, ?)
      ON CONFLICT(user_id) DO UPDATE SET locale = excluded.locale, updated_at = CURRENT_TIMESTAMP
    `);
    ...
    this.savePreferencesTransaction = db.transaction((userId, preferences) => {
      this.saveSettingStatement.run(userId, preferences.locale || this.defaultLocale);
      const enabledByType = {
    ...
    this.deliveryUsersStatement = db.prepare(`
      SELECT u.id, u.username, u.email, u.nickname, u.avatar_url,
        COALESCE(s.locale, ?) AS locale, 1 AS new_posts,
        CASE WHEN EXISTS (
      ...
      FROM users u
      LEFT JOIN user_subscription_settings s ON s.user_id = u.id
    ...
    this.recipientStatement = db.prepare(`
      SELECT u.id, u.username, u.email, u.nickname, u.avatar_url, COALESCE(s.locale, ?) AS locale
      FROM users u
      LEFT JOIN user_subscription_settings s ON s.user_id = u.id
  ...
  migrateLegacyPreferences() {
    ...
    this.db.transaction(() => {
      this.db.exec(`
        INSERT INTO user_subscription_settings (user_id, locale, created_at, updated_at)
        SELECT user_id, locale, created_at, updated_at FROM user_subscriptions
        WHERE 1
        ON CONFLICT(user_id) DO UPDATE SET locale = excluded.locale, updated_at = excluded.updated_at;
        INSERT OR IGNORE INTO user_subscription_opt_outs (user_id, subscription_type)
        SELECT user_id, 'new_posts' FROM user_subscriptions WHERE new_posts = 0;
        INSERT OR IGNORE INTO user_subscription_opt_outs (user_id, subscription_type)
        SELECT user_id, 'newsletter' FROM user_subscriptions WHERE newsletter = 0;
        INSERT OR IGNORE INTO user_subscription_opt_outs (user_id, subscription_type)
        SELECT user_id, 'events' FROM user_subscriptions WHERE events = 0;
        DROP TABLE user_subscriptions;
      `);
    })();
  }

  getPreferences(userId, fallbackLocale = this.defaultLocale) {
    const setting = this.getSettingStatement.get(userId);
    const optOuts = new Set(this.getOptOutsStatement.all(userId).map(row => row.subscription_type));
    return {
      locale: setting?.locale || fallbackLocale,
